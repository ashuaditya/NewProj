...
public class ProjectController {
	...
	public String saveData(@ModelAttribute ProjectDetails pd, Model model,MultipartFile file){
		...
		List<Billing> bill =  pd.getBill();
		for (Billing bl : bill)
		{
			if(bl != null) 
			{
				/*if(bl.getBillNumber() != null) {
						System.out.println("Bill Number="+bl.getBillNumber());
					}
					if(bl.getBillDate() != null) {
						System.out.println("Bill Date="+bl.getBillDate());
					}
					if(bl.getBillAmount() != null) {
						System.out.println("Bill Amount="+bl.getBillAmount());
					}
					if(bl.getFile() != null) {
						System.out.println("Bill File="+bl.getFile());
					}System.out.println("==============");
				 */
